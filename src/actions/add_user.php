...
$data = getPost(["name", "firstname", "email", "user_type"]);
var_dump(checkEmail($data["email"]));
var_dump($data);
if (checkLenString($data["name"], 32) && checkLenString($data["firstname"], 32) && checkLenString($data["email"], 320) && checkEmail($data["email"])
    ...
    if (sqlCommand("SELECT count(id) FROM utilisateur WHERE email=:email", [":email" => $data["email"]], $conn)[0][0] != 0) {
        ...
        sqlCommand("INSERT INTO utilisateur (nom, prenom, email, type_utilisateur) VALUES (:name,:firstname,:email,:type)", [":name" => $data["name"], ":firstname" => $data["firstname"], ":email" => $data["email"], ":type" => $data["user_type"]], $conn);
