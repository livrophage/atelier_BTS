...
if (isAdmin() == false) {
    ...
    if (isset($id) == true) {
        ...
        if (count($data) == 0) {//vérifie si l'étude existe
            header("Location: ./study.php"); //si non, la page se rafraichit sans l'id étude et donc la page pour créer une étude
            exit();
        }
        $id = $data["id"];
        ...
        <section id="home-hero">
            <div class="container my-5 border border-4 px-5 py-5 bg-light">
                <div class="container-form">
                    <form class="needs-validation" novalidate action="../src/actions/add_modify_studie.php" id="studie"
                          name="studie"
                        ...
                        <fieldset class="form-control"><!--sélection des plages étudiées-->
                            ...
                            if (isset($data["id"])) {
                                echo "<p class='text-danger fw-bold mt-2'>Lorsqu'une plage est retiré de l'étude, les données associées à celle-ci sont supprimées</p>";
                            }
