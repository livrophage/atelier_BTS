...
//récupération du nouveau nom de la plage
    ...
    if (checkLenString($new_name, 32) && sqlCommand("SELECT count(id) FROM communes WHERE id=:id", [":id" => $id], $conn)[0][0] == 1) {
        sqlCommand("UPDATE communes SET nom=:name WHERE id=:id", [":name" => $new_name, ":id" => $id], $conn,false);
